...
server <- function(input, output) {
  ...
  output$access1 <- renderPlot({
    ...
    plot11 <- cleaned_root %>% 
      ...
      ylab("Depth (cm)") + 
      theme_set(theme_cowplot()) + theme_tsl() + theme(legend.position = "none")
    plot15_stock <- cleaned_root %>% 
      filter(accession == input$access1,
             experiment %in% c("HL15")) %>% 
      ggplot(aes(start_x,start_y)) + 
      geom_segment(aes(xend=end_x,yend=end_y, color=hours),arrow = arrow(length = unit(0.01, "npc")),alpha=1) + 
      xlim(0,15) + 
      scale_y_reverse(limits=c(30,0)) + scale_color_gradient(low="darkred", high="gold", limits = c(0, 768)) + 
      ...
      xlab("Width (cm)") + 
      ylab("Depth (cm)") + 
      theme_set(theme_cowplot()) + theme_tsl()
    plot15 <- plot15_stock + theme(legend.position = "none")
    legend <- get_legend(plot15_stock + theme(legend.position = "bottom"))
    plots <- plot_grid(plot6, plot11, plot15, ncol = 3)
    plotall <- plot_grid(plots, legend, nrow = 2, rel_heights = c(1, 0.2))
    ...
    plot_grid(plotall, plotrep, nrow =1, rel_widths = c(2, 3))
  })
